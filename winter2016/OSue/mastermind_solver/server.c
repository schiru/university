...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    if (sockfd == -1) {
        bail_out(EXIT_FAILURE, "Failed to create socket");
    }

    // Prevent error "Address already in use" when binding
    int optval = 1;
    if (-1 == setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof optval)) {
        bail_out(EXIT_FAILURE, "Failed to set opt SO_REUSEADDR");
    }

    struct sockaddr_in socketAddr;
    (void) memset(&socketAddr, 0, sizeof socketAddr);

    socketAddr.sin_family = AF_INET;
    socketAddr.sin_port = htons(options.portno);
    socketAddr.sin_addr.s_addr = inet_addr("127.0.0.1");

    if (-1 == bind(sockfd, (struct sockaddr *)&socketAddr, sizeof socketAddr)) {
        bail_out(EXIT_FAILURE, "Failed to bind socket");
    }

    if (-1 == listen(sockfd, BACKLOG)) {
        bail_out(EXIT_FAILURE, "Failed to start listening");
    }

    // param 2 contains information about the connecting client, (par 3 is sizeof par 2)
    connfd = accept(sockfd, NULL, NULL);

    if (connfd == -1) {
        bail_out(EXIT_FAILURE, "Failed to establish connection to client");
    }
    ...
    for (round = 1; round <= MAX_TRIES && !quit; ++round) {
        ...
        if (-1 == send(connfd, &buffer[0], 1, /* flags */ 0)) {
            bail_out(EXIT_FAILURE, "Failed to send message to client");
        }
